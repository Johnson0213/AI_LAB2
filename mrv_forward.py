...
def find_min(newTable,hints,unsigned):
    ...
    # print(hints)
    for hint in hints:
        if newTable[hint[0]][hint[1]]==0: # no mine
            for i in range(-1,2):
                for j in range(-1,2):
        ...
        if newTable[hint[0]][hint[1]]==1:# mine
            current_x=-1
            current_y=-1
            for i in range(-1,2):
                for j in range(-1,2):
                    if(hint[0]+i>=0 and hint[0]+i<len(newTable[0]) and hint[1]+j>=0 and hint[1]+j<len(newTable)):# not out of board
                        if [hint[0]+i,hint[1]+j] in unsigned:
                            if current_x==-1:
                                current_x=hint[0]+i
                                current_y=hint[1]+j
